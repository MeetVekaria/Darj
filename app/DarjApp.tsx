...
type Screen = 'login' | 'reviewer' | 'filings' | 'newFiling' | 'services' | 'company' | 'documents' | 'payments' | 'guidance' | 'about' | 'prepare' | 'jaanch' | 'mohar' | 'sign' | 'rasid' | 'status' | 'recovery' | 'lineage' | 'evidence' | 'limitations' | 'demoControls';
    ...
    <div className="app-shell">
      ...
      <div className="platform-frame">
        ...
        <main id="main-content" className="app-main">
          ...
          {screen === 'services' && <ServiceDirectoryScreen query={serviceQuery} category={serviceCategory} selectedKey={selectedServiceKey} onQuery={setServiceQuery} onCategory={setServiceCategory} onSelect={setSelectedServiceKey} onOpenWorking={() => navigate(resumeScreen(state))} onStart={() => navigate('newFiling')} />}
      ...
      <footer className="app-footer">
        <span>DARJ / दर्ज. Independent MCA21 filing workspace.</span>
        <nav aria-label="Footer"><button onClick={() => navigate('reviewer')}>Reviewer guide</button><button onClick={() => navigate('evidence')}>Evidence</button><button onClick={() => navigate('limitations')}>Limitations</button>{state.features.correctionLineage && <button onClick={() => navigate('lineage')}>Package lineage</button>}<button onClick={() => void resetDemo()} disabled={busy === 'reset'}>Reset review workspace</button><button onClick={() => void signOut()} disabled={busy === 'logout'}>Sign out</button></nav>
      </footer>
    </div>
  );
}
...
function Disclosure({ onOpen }: { onOpen: () => void }) {
  return <button className="prototype-strip disclosure-button" onClick={onOpen}>INDEPENDENT MCA21 FILING WORKSPACE · SAMPLE DATA · NOT AFFILIATED WITH MCA</button>;
}
...
function AppHeader({ screen, state, theme, onTheme, serviceQuery, onServiceQuery, onNavigate, onSignOut, signingOut }: { screen: Screen; state: AppState; theme: Theme; onTheme: () => void; serviceQuery: string; onServiceQuery: (value: string) => void; onNavigate: (screen: Screen) => void; onSignOut: () => void; signingOut: boolean }) {
  return (
    <header className="app-header">
      ...
      <div className="header-actions"><ThemeToggle theme={theme} onTheme={onTheme} />{screen !== 'filings' && <button className="text-button filing-register-link" onClick={() => onNavigate('filings')}>Filing register</button>}<button className="text-button signout-button" onClick={onSignOut} disabled={signingOut}>{signingOut ? 'Signing out…' : 'Sign out'}</button></div>
    </header>
  );
}
...
function PlatformNav({ screen, onNavigate }: { screen: Screen; onNavigate: (screen: Screen) => void }) {
  const items: Array<{ screen: Screen; label: string; index: string }> = [
    ...
    { screen: 'newFiling', label: 'New filing', index: '02' },
...
function FilingsScreen({ state, onPrepare, onRecovery, onNavigate }: { state: AppState; onPrepare: () => void; onRecovery: () => void; onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <section className="page-section dashboard-page" aria-labelledby="filings-title">
      <div className="dashboard-intro"><div><p className="eyebrow">MCA21 filing workspace</p><h1 id="filings-title">Good afternoon, Meet.</h1><p>Your company workspace keeps each filing, its evidence and its outcome in one traceable record.</p></div><button className="primary dashboard-start" onClick={() => onNavigate('newFiling')}>Start a new filing <span aria-hidden="true">→</span></button></div>
...
function ReviewerScreen({ theme, onTheme, onNavigate }: { theme: Theme; onTheme: () => void; onNavigate: (screen: Screen) => void }) {
  ...
  return <main className="reviewer-shell"><header className="reviewer-header"><button className="brand-button" onClick={() => onNavigate('login')} aria-label="DARJ home"><Wordmark /></button><div><button className="text-button" onClick={() => onNavigate('login')}>Home</button><ThemeToggle theme={theme} onTheme={onTheme} /></div></header><section className="reviewer-page" aria-labelledby="reviewer-title"><div className="reviewer-intro"><p className="eyebrow">REVIEWER GUIDE</p><h1 id="reviewer-title">Five recovery paths. Five minutes.</h1><p>Each link opens the exact place worth reviewing. DARJ signs you into an isolated sample workspace automatically.</p></div><div className="reviewer-links">{paths.map(([title, path, proof], index) => <a href={path} key={path}><span className="mono">{String(index + 1).padStart(2, '0')}</span><strong>{title}</strong><small>{proof}</small><span aria-hidden="true">→</span></a>)}</div><div className="reviewer-boundary"><strong>Independent review environment</strong><p>Sample data only. DARJ does not connect to MCA21, make a statutory filing or collect sensitive identifiers.</p></div></section></main>;
}
...
function PublicInformationScreen({ screen, theme, onTheme, onNavigate }: { screen: 'evidence' | 'limitations'; theme: Theme; onTheme: () => void; onNavigate: (screen: Screen) => void }) {
  return <div className="app-shell public-shell"><Disclosure onOpen={() => onNavigate('limitations')} /><header className="app-header"><button className="brand-button" onClick={() => onNavigate('login')} aria-label="DARJ login"><Wordmark compact /></button><div className="header-context"><span className="mono">PUBLIC RECORD</span><strong>{screen === 'evidence' ? 'Evidence' : 'Limitations'}</strong><span>Independent MCA21 filing workspace</span></div><div className="header-actions"><ThemeToggle theme={theme} onTheme={onTheme} /><button className="text-button" onClick={() => onNavigate('login')}>Open workspace</button></div></header><main id="main-content" className="app-main">{screen === 'evidence' ? <EvidenceScreen /> : <LimitationsScreen />}</main><footer className="app-footer"><span>DARJ / दर्ज. Independent MCA21 filing workspace.</span><nav aria-label="Footer"><button onClick={() => onNavigate('reviewer')}>Reviewer guide</button><button onClick={() => onNavigate('evidence')}>Evidence</button><button onClick={() => onNavigate('limitations')}>Limitations</button><button onClick={() => onNavigate('login')}>Open workspace</button></nav></footer></div>;
}
...
function pathForScreen(screen: Screen, caseId = 'DARJ-DEMO-AOC4-01') {
  return ({
    login: '/login', reviewer: '/reviewer', filings: '/filings', newFiling: '/filings/new', services: '/services', company: '/company', documents: '/documents', payments: '/payments', guidance: '/guidance', about: '/about', prepare: `/filings/${caseId}/prepare`, jaanch: `/filings/${caseId}/jaanch`,
