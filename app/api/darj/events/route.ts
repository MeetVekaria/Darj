...
export async function GET(request: Request) {
  ...
  if (!runId || !(await activeRun(runId))) return NextResponse.json({ error: { code: 'DARJ_AUTH_REQUIRED', stage: 'LOGIN', blocking: true, retryable: true, summary: 'Re-enter the DARJ demo.', detail: 'Your local draft is unchanged.', correlationId: `DARJ-CORR-${crypto.randomUUID().slice(0, 8).toUpperCase()}` } }, { status: 401 });
