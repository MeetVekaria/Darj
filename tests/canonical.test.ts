...
test('canonical output is stable across key insertion order', async () => {
  const first = { packageId: 'DARJ-PKG-000023', form: { z: 'last', a: 'first' }, version: 23 };
  const second = { version: 23, form: { a: 'first', z: 'last' }, packageId: 'DARJ-PKG-000023' };
...
test('sensitive real-looking identifiers are rejected while demo handles pass', () => {
  assert.equal(containsRealLookingSensitiveIdentifier({ id: 'DARJ-DEMO-CIN-000117' }), false);
